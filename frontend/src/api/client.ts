...
export const api = {
  ...
  sendMessage: (conversationID: string, content: string, clientMessageId: string, mode: FollowUpMode = "queue") =>
    request<{ messageId: string; runId: string; status: string }>(`/conversations/${conversationID}/messages`, {
      method: "POST",
      body: JSON.stringify({ content, clientMessageId, mode }),
    }),
  cancelActive: (conversationID: string) =>
    request<{ cancelled: boolean }>(`/conversations/${conversationID}/cancel-active`, { method: "POST" }),
